	...
	public static Iterator<Object[]> CreateData1(Method m) throws ClassNotFoundException, SQLException {
		...
		if (m.getName().contains("TrailerWithoutPRO")) {
			...
			Query = DataCommon.query2;
				...
				|| m.getName().equalsIgnoreCase("LDDTrailerHasProChangeNonDestinationField")) {
			Query = DataCommon.query5;
		} else if (m.getName().equalsIgnoreCase("LDDTrailerNoProChangeDestination")
				|| m.getName().equalsIgnoreCase("LDDTrailerNoProChangeNonDestinationField")) {
