...
public class US470LoadProFromOtherTrailer {
  ...
  public void NoProTrailerPullProFromTrailerInOtherTerminal(String terminalcd,String SCAC,String TrailerNB,String Desti) throws AWTException, InterruptedException, ClassNotFoundException, SQLException, ParseException {
	  ...
	  ArrayList<ArrayList<String>> PROInformation=DataForUS470.GetProFromTrailerOnDifferentTerminal(terminalcd, SCAC, TrailerNB);
	 ...
	 (new WebDriverWait(driver, 50)).until(ExpectedConditions.invisibilityOfElementLocated(By.xpath("html/body/div[4]/div/div")));
	  ...
	  (new WebDriverWait(driver, 80)).until(ExpectedConditions.invisibilityOfElementLocated(By.xpath("html/body/div[1]/div")));
	  page.EnterTrailer(SCAC,TrailerNB);
	  (new WebDriverWait(driver, 150)).until(ExpectedConditions.textToBePresentInElement(page.ProListForm, CurrentProH));
	  int NEW2=page.ProListForm.findElements(By.xpath("div")).size();
	  Sassert.assertEquals(page.ProListForm.findElement(By.xpath("div["+NEW2+"]/div/div[2]")).getText(),CurrentProH);
	  ...
	  Sassert.assertEquals(AfterADDWb.get(2),BeforeADDWb.get(2),"Waybill table Destination_facility_cd is wrong "+CurrentPro);
	  Sassert.assertEquals(AfterADDWb.get(3),"LH.LDG","Waybill table Source_Modify_ID is wrong "+CurrentPro);
	  Sassert.assertEquals(AfterADDWb.get(4),BeforeADDWb.get(4),"Waybill table M204_SHP_TAG_ID is wrong "+CurrentPro);
	  Sassert.assertEquals(AfterADDWb.get(5),BeforeADDWb.get(5),"Waybill table M204_WGP_TAG_ID is wrong "+CurrentPro);
	  Sassert.assertEquals(AfterADDWb.get(6),BeforeADDWb.get(6),"Waybill table Record_key is wrong "+CurrentPro);
	  Sassert.assertEquals(AfterADDWb.get(7),BeforeADDWb.get(7),"Waybill table Create_TS is wrong "+CurrentPro);
	  Sassert.assertEquals(AfterADDWb.get(8),BeforeADDWb.get(8),"Waybill table System_Insert_TS is wrong "+CurrentPro); 
	  ...
	  Sassert.assertEquals(AfterADDWb.get(12),BeforeADDWb.get(12),"Waybill table Record_KEY is wrong "+CurrentPro); 
      //loading record
 ...
 @Test(priority=2,dataProvider = "4.70",dataProviderClass=DataForUS470.class)
  public void WithProTrailerPullProFromTrailerInOtherTerminal(String terminalcd,String SCAC,String TrailerNB,String Desti) throws AWTException, InterruptedException, ClassNotFoundException, SQLException, ParseException {
	  SoftAssert Sassert= new SoftAssert();   
	  page.SetLocation(terminalcd);	 
	  page.EnterTrailer(SCAC,TrailerNB);
	  String Cube=page.CubeField.getAttribute("value");	 
  ...
  public void NoProTrailerPullProFromTrailerInSameTerminal(String terminalcd,String SCAC,String TrailerNB,String Desti) throws AWTException, InterruptedException, ClassNotFoundException, SQLException, ParseException {
	  SoftAssert Sassert= new SoftAssert();   
	  page.SetLocation(terminalcd);	 
	  page.EnterTrailer(SCAC,TrailerNB);		 
	  String Cube=page.CubeField.getAttribute("value");
	  ...
	  ArrayList<ArrayList<String>> PROInformation= DataForUS470. GetProFromTrailerOnSameTerminal(terminalcd, SCAC, TrailerNB);
		 page.RemoveProButton.click();
		 for(int j=0;j<1;j++){
	     String CurrentPro=PROInformation.get(j).get(0);
	     String FromSCAC=PROInformation.get(j).get(1);
  ...
  public void WithProTrailerPullProFromTrailerInSameTerminal(String terminalcd,String SCAC,String TrailerNB,String Desti) throws AWTException, InterruptedException, ClassNotFoundException, SQLException, ParseException {
	SoftAssert Sassert= new SoftAssert();  
	page.SetLocation(terminalcd);	 
	page.EnterTrailer(SCAC,TrailerNB);
	String Cube=page.CubeField.getAttribute("value");			 
