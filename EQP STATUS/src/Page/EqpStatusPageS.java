...
(new WebDriverWait(driver, 2)).until(ExpectedConditions.visibilityOfElementLocated(By.linkText(FullStatusName)));
//(new WebDriverWait(driver, 2)).until(ExpectedConditions.textToBePresentInElement(this.TrailerField, FullStatusName));
...
public void EnterTrailer(String SCAC, String TrailerNB) throws AWTException, InterruptedException{
	Actions builder = new Actions(driver);
	if(TrailerField.isDisplayed()){this.TrailerField.click(); }
	this.TrailerInputField.click();
    this.TrailerInputField.clear();
    this.TrailerInputField.sendKeys(TrailerNB);
    this.TrailerInputField.click();
    (new WebDriverWait(driver, 2)).until(ExpectedConditions.textToBePresentInElementValue(this.TrailerInputField, TrailerNB));
    if(driver instanceof InternetExplorerDriver ){  builder.sendKeys(this.TrailerInputField,Keys.SPACE,Keys.SPACE,Keys.SPACE,Keys.TAB).build().perform();}
    else{builder.sendKeys(this.TrailerInputField,Keys.TAB).build().perform(); }
   // builder.sendKeys(this.TrailerInputField,Keys.TAB).build().perform();
    String SCACTrailer=SCACTrailer( SCAC, TrailerNB);
    try{
	(new WebDriverWait(driver, 2)).until(ExpectedConditions.textToBePresentInElement(this.TrailerField, SCACTrailer));
		//TimeoutException
	}catch(Exception e){
	(new WebDriverWait(driver, 20)).until(ExpectedConditions.visibilityOfElementLocated(By.linkText(SCACTrailer)));							
	driver.findElement(By.linkText(SCACTrailer)).click();	
	} 
	(new WebDriverWait(driver, 5)).until(ExpectedConditions.textToBePresentInElement(this.TrailerField, SCACTrailer)); 
	Thread.sleep(1000);
	(new WebDriverWait(driver, 20)).until(ExpectedConditions.invisibilityOfElementLocated(By.id("loading-bar")));
	 }
...
public void HandleLOBRproAll(String handle) throws InterruptedException{
	Actions builder = new Actions(driver);
		try{ if(this.LeftoverCheckAllPRO.isDisplayed()){
		...
		(new WebDriverWait(driver, 50)).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("html/body/div[4]/div/div")));
...
String DateAndTime=this.DateInput.getAttribute("value")+" "+this.HourInput.getAttribute("value")+":"+this.MinuteInput.getAttribute("value");
...
public void SetDatePicker2(Date SetDate,int AlterHour,int AlterMinute){
	...
	SimpleDateFormat dateFormat = new SimpleDateFormat("MM/dd/YYYY");
	cal.setTime(SetDate);
	if(AlterHour!=0) cal.add(Calendar.HOUR_OF_DAY,AlterHour);
	if(AlterMinute!=0) cal.add(Calendar.MINUTE, AlterMinute);
	int hourOfDay = cal.get(Calendar.HOUR_OF_DAY); // 24 hour clock
	String hour=String.format("%02d",hourOfDay);
	int minute = cal.get(Calendar.MINUTE);
	String Minute=String.format("%02d",minute);
...
public LinkedHashSet<ArrayList<String>> GetProList(WebElement ProGrid){
	...
	String[] Proline1= ArrayUtils.remove(ProGrid.findElement(By.xpath("div["+i+"]")).getText().split("\\n"), 0);
	//String[] Proline1= page.ProListForm.findElement(By.xpath("div["+i+"]")).getText().split("\\n");
	...
	int additional=31;
	do{
		jse.executeScript("arguments[0].scrollIntoView(true);",ProGrid.findElement(By.xpath("div["+additional+"]")));
		additional=ProGrid.findElements(By.xpath("div")).size();
		for(int j=1;j<=additional;j++) {
		String[] Proline1= ArrayUtils.remove(ProGrid.findElement(By.xpath("div["+j+"]")).getText().split("\\n"), 0);
		ArrayList<String> e1= new ArrayList<String> (Arrays.asList(Proline1));
		ProInfo.add(e1);}
		}
	while(additional>31);
