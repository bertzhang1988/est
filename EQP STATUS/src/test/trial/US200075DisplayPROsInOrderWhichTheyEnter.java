...
public class US200075DisplayPROsInOrderWhichTheyEnter {
 ...
 // @Test(priority=1,dataProvider = "2000.75",dataProviderClass=DataForUS200075.class, groups = { "ldg uc" })
  ...
  @Test(priority=4,dataProvider = "12.15",dataProviderClass=DataForUS1215.class,description="non ldg without pro, add pro through quick close", groups = { "ldg uc" })
  public void NONLDGTrailerWithoutPROAddPRO(String terminalcd,String SCAC,String TrailerNB,String Desti,String AmountPro,String AmountWeight,String Cube, String seal,String hldesti, String hlcube, Date MReqpst) throws AWTException, InterruptedException, ClassNotFoundException, SQLException{	
   ...
   //  (new WebDriverWait(driver, 20)).until(ExpectedConditions.invisibilityOfElementLocated(By.id("loading-bar")));
    ...
    Thread.sleep(20000);
     ...
     SAssert.assertTrue(Math.abs(System_Modify_TS.getTime()-d.getTime())<120000,""+pro +" waybill table System_Modify_TS "+System_Modify_TS+"  "+d);
     Date Waybill_Transaction_End_TS=CommonFunction.SETtime((Date) CheckWaybillRecord.get(11));	
     SAssert.assertTrue(Math.abs(Waybill_Transaction_End_TS.getTime()-d.getTime())<120000,""+pro +" waybill table Waybill_Transaction_End_TS "+System_Modify_TS+"  "+d);
     }
...
//add no waybill record pro
 ...
 // LinkedHashSet<ArrayList<String>> ProInfo=page.GetProList(page.ProListForm);
  ...
  SAssert.assertEquals(NewWbAndWbtRecord.get(1), TrailerNB,""+CurrentPro +"wb.Equipment_Unit_NB is wrong");
  SAssert.assertEquals(NewWbAndWbtRecord.get(17), SCAC,""+CurrentPro +"wb.To_Standard_Carrier_Alpha_CD is wrong");
  SAssert.assertEquals(NewWbAndWbtRecord.get(18), TrailerNB,""+CurrentPro +"wb.To_Equipment_Unit_NB is wrong");
  SAssert.assertEquals(NewWbAndWbtRecord.get(20), "LOADING",""+CurrentPro +"wb.Waybill_Transaction_Type_NM is wrong");
  Date System_Modify_TS=CommonFunction.SETtime((Date) NewWbAndWbtRecord.get(9));	
  SAssert.assertTrue(Math.abs(System_Modify_TS.getTime()-d.getTime())<120000,""+CurrentPro +" waybill table System_Modify_TS "+System_Modify_TS+"  "+d);
  Date Waybill_Transaction_End_TS=CommonFunction.SETtime((Date) NewWbAndWbtRecord.get(11));	
  SAssert.assertTrue(Math.abs(Waybill_Transaction_End_TS.getTime()-d.getTime())<120000,""+CurrentPro +" waybill table Waybill_Transaction_End_TS "+Waybill_Transaction_End_TS+"  "+d);}
 // System.out.println((d2.getTime()-d.getTime())/1000.0);
  SAssert.assertAll();
  }
