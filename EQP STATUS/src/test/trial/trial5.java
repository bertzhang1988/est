...
public class trial5 {
	...
	public void ef() throws AWTException, InterruptedException {
		...
		for (int i = 1; i <= line; i++) {
			// String[] Proline1=
			// ArrayUtils.remove(page.ProListForm.findElement(By.xpath("div["+i+"]")).getText().split("\\n"),
			// 0);
		...
		if (line >= 31) {
			...
			do {
				...
				for (int j = 1; j <= additional; j++) {
					String[] Proline1 = page.ProListSecondForm.findElement(By.xpath("div[" + j + "]")).getText()
							.split("\\n");
					ArrayList<String> e1 = new ArrayList<String>(Arrays.asList(Proline1));
					ProInfo.add(e1);
				}
		...
		// SA.assertEquals(ProInfo,DataForUS461AndUS465AndUS457.GetProList(SCAC,
		// TrailerNB));
