...
public void VerifyCubePatternCheck(String terminal, String scac,String trailernb) throws InterruptedException, AWTException{
	...
	String[] Cube={"00011","1 1","00","   1","33","88","100","1030","7000","abc"};
	 ...
	 if (result==2){
(new WebDriverWait(driver, 10)).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[contains(text(), 'Invalid Cube. Cube must be between 1 and 100.')]")));
		}else{
(new WebDriverWait(driver, 10)).until(ExpectedConditions.invisibilityOfElementLocated(By.xpath("//*[contains(text(), 'Invalid Cube. Cube must be between 1 and 100.')]")));		
		}
...
//@Test(priority=3,dataProvider = "12.05",dataProviderClass=DataForUS1205.class)
public void VerifyMannuallyInputShipmentCount(String terminal, String scac,String trailernb) throws InterruptedException, AWTException{
	 page.SetLocation(terminal);
	 page.EnterTrailer(scac, trailernb); 
...
public void VerifySealPatternCheck(String terminal, String scac,String trailernb) throws InterruptedException, AWTException{
	page.SetLocation(terminal);
	page.EnterTrailer(scac, trailernb); 
	...
	if (result<4){
(new WebDriverWait(driver, 10)).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[contains(text(), 'Invalid Seal Number. The Seal Number must be at least 4 characters.')]")));
	}else {Assert.assertFalse(page.CheckErrorAndWarningMessageDisplay("Invalid Seal Number. The Seal Number must be at least 4 characters."));}
	 }}
